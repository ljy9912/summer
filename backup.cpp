...
void BackUp::SignUpForLeader(QString ID,int idTask){
    ...
    Message myMessage(m_List.message.last().GetID()+1);
    myMessage.EditTitle("报名负责人成功！");
    int iNum=m_List.searchTaskInList(idTask);
    myMessage.EditContent(QObject::tr("%1\n您刚才报名了%2翻译任务的负责人，请耐心等待发布者审核与选择！").arg(time.toString()).arg(m_List.TaskPublisher[iNum].GetIntroduction()));
...
void BackUp::TaskPublish(QString ID, QString intro){
    QDateTime time=QDateTime::currentDateTime();
    Message myMessage;
    if(m_List.message.isEmpty()){
        myMessage.EditID(0);
    }
    else{
        myMessage.EditID(m_List.message.last().GetID()+1);
    }
...
void BackUp::CheckDateSnupfrLeader(){
    ...
    for(int i=0;i<m_List.TaskPublisher.size();i++){
        ...
        if(date1<currentdate&&m_List.TaskPublisher[i].GetFlag()==101){
            Message newMessage(m_List.message.last().GetID()+1);
            ...
            newMessage.EditContent(QObject::tr("%4\n负责人报名已于%1年%2月%3日结束，快点击“我是发布者”选择负责人吧！").
                                   ...
                                   .arg(m_List.TaskPublisher[i].GetLeaderDay()).arg(time.toString()));
...
void BackUp::CheckDateSnupfrTranslater(){
    ...
    for(int i=0;i<m_List.TaskLeader.size();i++){
        ...
        QDate currentdate;
        currentdate=QDate::currentDate();
        if(date1<currentdate&&m_List.TaskLeader[i].GetFlag()==202){
            Message newMessage(m_List.message.last().GetID()+1);
            ...
            newMessage.EditContent(QObject::tr("%4\n译者报名已于%1年%2月%3日结束，快点击“我是负责人”选择译者并分配任务吧！").
                                   ...
                                   .arg(m_List.TaskLeader[i].GetTranslaterDay()).arg(time.toString()));
...
void BackUp::SelectLeaderDone(QString PublisherID, QString idLeader, QString intro){
    ...
    m_List.message.append(newMessage);
    newMessage.EditID(m_List.message.last().GetID()+1);
